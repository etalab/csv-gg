    ...
    <div style="min-height: 500px" >
        ...
        <div v-if="schema && !publicationReady" class="rf-container rf-pb-6w rf-pt-2w">
            ...
            <div v-if="report">
                ...
                <div v-if="reportErrorInfo"><br/>La tâche de validation a échouée. {{ reportErrorInfo }}.</div>
                <br/>
                <br/><br/>
                ...
                <vsa-item v-if="reportStructureErrors.length != 0">
                    ...
                    <vsa-content>
                        <ul v-for="error in reportStructureErrors">
                ...
                <vsa-item v-if="reportContentErrors.length != 0" >
                    ...
                    <vsa-content>
                        <ul v-for="error in reportContentErrors">
                            <li><b>{{ error.name }}</b> : <span v-if="error.fieldName">colonne <i>{{ error.fieldName }}</i>,</span> rang {{ error.rowPosition }}</li>
                        </ul>
                    </vsa-content>
                </vsa-item>
            ...
            <div v-if="publication">
                <button type="submit" class="rf-btn" title="Publier sur datagouv" @click="publicationForm()">{{ publicationMessage }}</button>
            </div>
        </div>
...
export default {
  ...
  methods: {
    handleFileUpload(){
        ...
        //formData.append('schema', "https://raw.githubusercontent.com/etalab/schema-irve/master/schema.json");
        ...
        .then(data => {
            ...
            if(data.report.errors.length > 0){
                ...
                if(data.report.tables[0].errors){
  ...
  mounted() {
      ...
      fetch("https://schema.data.gouv.fr/schemas/"+this.schemaName+"/latest/schema.json").then(r => {
          return r.json()
      }).then(data => {
